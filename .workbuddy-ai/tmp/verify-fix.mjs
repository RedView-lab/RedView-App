// Verify: does supersampled WMS + NaN-aware box average kill the row-duplication
// and the Horn dash artifact on the LiDAR-HD MNS layer?
...
async function get(u, tries = 8) {
  for (let i = 0; i < tries; i++) {
    ...
    const b = await r.arrayBuffer(); return new Float32Array(b);
  }
  console.log('  rate limited'); return null;
}
...
function slopeRgba(own, z, x, y) {
  const b = bounds(z, x, y), mid = (b.north + b.south) / 2, lr = mid * Math.PI / 180;
...
function eqY(f, n) { let e = 0, t = 0; for (let y = 1; y < n; y++) for (let x = 0; x < n; x++) { const i = y * n + x; t++; if (f[i] === f[i - n]) e++; } return 100 * e / t; }
function eqX(f, n) { let e = 0, t = 0; for (let y = 0; y < n; y++) for (let x = 1; x < n; x++) { const i = y * n + x; t++; if (f[i] === f[i - 1]) e++; } return 100 * e / t; }
function slopeStats(f) {
  let sum = 0, n = 0, hi = 0;
  for (let y = 1; y < S - 1; y++) for (let x = 1; x < S - 1; x++) {
    const i = y * S + x; const gy = (f[i + S] - f[i - S]) / 2, gx = (f[i + 1] - f[i - 1]) / 2;
    const g = Math.hypot(gx, gy) / 6.75; const deg = Math.atan(g) * 180 / Math.PI;
    sum += deg; n++; if (deg > 60) hi++;
  }
  return { mean: sum / n, pct60: 100 * hi / n };
}
...
for (const ss of [1, 2, 4]) {
  const n = S * ss;
  const hi = await get(url(t.z, t.x, t.y, LIDAR, n, n));
  if (!hi) continue;
  let out;
  if (ss === 1) out = hi;
  else {
    ...
    for (let y = 0; y < S; y++) for (let x = 0; x < S; x++) {
      ...
      for (let yy = 0; yy < ss; yy++) for (let xx = 0; xx < ss; xx++) {
        ...
        if (!Number.isNaN(v) && v >= MINV && v <= MAXV) { sum += v; c++; }
      }
      out[y * S + x] = c > 0 ? sum / c : NaN;
    }
  }
  const st = slopeStats(out);
  console.log(`supersample=${ss} (${n}x${n} request)  eqY=${eqY(hi, n).toFixed(2)}%  eqX=${eqX(hi, n).toFixed(2)}%  |  out 256 eqY=${eqY(out, S).toFixed(2)}%  slope mean=${st.mean.toFixed(1)}deg  %>60deg=${st.pct60.toFixed(2)}%`);
  writeFileSync(`${OUT}/fix-ss${ss}.png`, png(S, S, slopeRgba(out, t.z, t.x, t.y)));
  writeFileSync(`${OUT}/fix-ss${ss}-x2.png`, png(S * 2, S * 2, mag2(slopeRgba(out, t.z, t.x, t.y))));
  await sleep(1500);
}
console.log('done');
