...
async function fetchMapboxTile(z: number, x: number, y: number, token: string): Promise<Buffer | null> {
...
// ---------------------------------------------------------------------------
// Public API — process a DEM tile request
// ---------------------------------------------------------------------------
...
/**
 * Process a DEM tile request. Returns the PNG buffer with status 200,
 * or null if no data is available (caller should return 204).
 */
export async function processDemTile(
  z: number,
  x: number,
  y: number,
  mapboxToken: string,
): Promise<DemTileResult | null> {
  if ([z, x, y].some((v) => Number.isNaN(v) || v < 0)) {
    return null;
  }
  ...
  if (tileOverlapsFrance(z, x, y) && z >= IGN_DEM_MINZOOM) {
    try {
      tileData = await buildIGNTile(z, x, y);
    } catch (err) {
  ...
  if (!tileData) {
    tileData = await fetchMapboxTile(z, x, y, mapboxToken);
  }
  ...
  return { data: tileData, status: 200 };
}
