 ...
 * @returns {any}
 */
export function predict(fit_files, gpx_data, config, on_progress) {
    try {
        ...
        wasm.predict(retptr, ptr0, len0, ptr1, len1, addHeapObject(config), isLikeNone(on_progress) ? 0 : addHeapObject(on_progress));
 ...
 * @returns {any}
 */
export function predict_vs_actual(training_fits, validation_fit, config) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passArrayJsValueToWasm0(training_fits, wasm.__wbindgen_export);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passArray8ToWasm0(validation_fit, wasm.__wbindgen_export);
        const len1 = WASM_VECTOR_LEN;
        wasm.predict_vs_actual(retptr, ptr0, len0, ptr1, len1, addHeapObject(config));
        var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
        var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
        var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
        if (r2) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('redviewalgo_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
