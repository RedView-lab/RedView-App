...
export function detectCrs(minY: number, maxY: number, minX?: number, maxX?: number): DetectedCrs {
  ...
  if (meanY > 1_000_000 && meanY < 1_400_000) {
    if (minX === undefined || maxX === undefined) return 'CH1903_LV95';
    const meanX = (minX + maxX) / 2;
    if (meanX > 2_300_000 && meanX < 3_000_000) return 'CH1903_LV95';
  }
...
export function buildTileFileName(xKm: number, yKm: number, crs: DetectedCrs, altRef?: AltitudeRef): string {
  ...
  if (crs === 'CH1903_LV95') {
    ...
    // Uses underscore separator instead of "-" so the cache regex stays simple.
...
export function wgs84ToTileCoord(lon: number, lat: number): TileCoord {
  // Swiss coverage takes priority near the CH border so locations not yet
  // covered by IGN LiDAR HD still get a valid swissSURFACE3D tile.
